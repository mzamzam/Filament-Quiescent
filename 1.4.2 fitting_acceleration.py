...
ax.set_ylabel('Accel. (km $s^{-2}$)', fontsize=16)
ax.set_xlabel('Start time = {}'.format(start_time.strftime('%Y/%m/%d %H:%M:%S')), fontsize=16)

ax.text(dt_intensity.date[10],0.045,'(f)',fontsize=20)
...
plt.savefig(f'Results/fitting_acceleration/fig_6f_{dtdt}.png',bbox_inches='tight', dpi=300)
plt.show()
