    ...
    for i in range(edge_coord.shape[0] - 20, edge_coord.shape[0], 1):
        ...
        if t_minute_onset != np.nan:
            try:
                ...
                ax.imshow(image_binned, cmap=reversed_map, origin='lower', vmax=35,
                          extent=[x_lim[0], x_lim[1], 0, ang_sep_mid.h_arcsec.max()], aspect='auto')
                one_km_to_arcsec = 1 / one_arcsec_to_km
                h_arcsec_model = (
                                             h_Mm_model * 1000) * one_km_to_arcsec  ## only have fitting line in Mm, need to convert the to arcsec
...
ax3.yaxis.set_minor_locator(ticker.MultipleLocator(10))
os.makedirs('Results/ht_plot_comp', exist_ok=True)
plt.savefig(f'Results/ht_plot_comp/fig_6b_{dtdt}_{}.png',bbox_inches='tight', dpi=100)
plt.show()
'''
