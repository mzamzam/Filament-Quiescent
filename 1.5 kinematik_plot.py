...
ht_above = np.zeros([315,len(data_list)]) #315 is an array size of blue line
ht_below = np.zeros([299,len(data_list)]) #299 is an array of green line
...
def plot_6a(i, x): #type x='save_all' to save all figures, x = 'save_one' to save only one figure, otherwise won't save figure
    ...
    xe_above, ye_above = -895, -1200 #it should be offset by 2 degree
    xe_below, ye_below = -855, -1200 #it should be offset by 2 degree#it should be offset by 2 degree
    ...
    intensity_main = sunpy.map.sample_at_coords(aia_map, intensity_coords)
    ...
    if x == 'save_all':
        plt.savefig(f'{save_fd}/intensity_along_line/fig_6a/{AIA_304[-26:-5]}.png', bbox_inches='tight', dpi=200)
        plt.close()
    elif x == 'save_one':
        nama_file = f'fig_6a_{AIA_304[-26:-5]}.png'
        plt.savefig(f'{save_fd}/{nama_file}', bbox_inches='tight', dpi=200)
        print(f'1.1 Saving intenisty along line fig ("{save_fd}{nama_file}").')
        plt.close()
    plt.show()
    
    return aia_map, intensity_main, data_date, intensity_coords, ht, ht_above, ht_below, angular_separation

## uncomment the code below for resulting in images and intensity.npy.
# [height_prom(i, 'save_all') for i in range(len(data_list))]

## to load sdoaia304 cmap, only use aia_map variabel
aia_map, inte, dd, inco, ht, ht_ab, ht_b, ans =  plot_6a(example_case, 'save_on')
...
intensity = np.load(f"{in_fd}/intensity.npy") #output from height_prom(i):
intensity_ab = np.load(f"{in_fd}/intensity_above.npy") # for blue line
intensity_be = np.load(f"{in_fd}/intensity_below.npy") # for green line

dt_format = '%Y-%m-%dT%H:%M:%S.%f'
dt_intensity = pd.read_csv(f"{in_fd}/datetime_intensity.csv", index_col='Unnamed: 0',parse_dates=['date'], date_format=dt_format)
...
ang_sep = pd.read_csv(f"{in_fd}/angular_separation.csv")
...
imp_main = improve(intensity)
imp_ab = improve(intensity_ab)
imp_be = improve(intensity_be)
...
def plot_6b(x): #type x='save' to save figure, otherwise won't save anything
    ...
    ax.imshow(imp_main, cmap=reversed_map, origin='lower', vmax=35, extent=[x_lim[0],x_lim[1],0,ang_sep.h_arcsec.max()], aspect='auto')
    ax.set_ylim(0,470)
...
h_data_mm = df_coord_edge.y_Mm[:73] #different data slicing resulting in different result
...
def plot_6b_comp(x):
    ...
    '''
    fig, ax, ax2 = plot_param()
    h_data_arcsec = df_coord_edge.y_arcsec[:73]
    ...
    ax.plot(model_t[10:], h_model_arcsec[10:], color='k')
    ax.set_ylim(0,470)
    ax.set_ylabel('Distance along slice (arcsec)', fontsize=18)
    ax.set_xlabel('Start time = {}'.format(start_time.strftime('%Y/%m/%d %H:%M:%S')), fontsize=18)

    ax.text(dt_intensity.date[10],410,'(b)',fontsize=20)
    ax.text(onset_time-timedelta(minutes=45),onset_h_arcsec+13,f'{round(onset_h_Mm)} Mm',fontsize=13)
...
# xlim = [dt_intensity.date[0],dt_intensity.date[len(dt_intensity)-1]]
# x_lim = mdates.date2num(xlim)
# ax.imshow(imp_be, cmap=reversed_map, origin='lower', vmax=35, extent=[x_lim[0],x_lim[1],0,ang_sep.h_arcsec.max()], aspect='auto')
# ax.set_ylim(0,470)
...
# # plt.savefig('Results/height-time_plot/fig_6b_lite.png',bbox_inches='tight', dpi=100)
# plt.show()
...
# datet = pd.to_datetime(df.date)
# def v(i):
#     velocity = abs(df.height_km[i+1]-df.height_km[0])/abs((datet[i+1] - datet[0]).seconds)
#     df.loc[i, 'velocity'] = velocity
#     df.loc[i, 'delta_t'] = datet[i+1] - datet[0]
# [v(i) for i in range(151)]
# plt.rc('font', family='serif', serif='Times')
# plt.rc('text', usetex=True)
# plt.rc('xtick', labelsize=12)
# plt.rc('ytick', labelsize=12)
...
# ax1.plot(pd.to_datetime(df.date), df.height, '.')
# # ax1.set_ylim(0,420)
# ax1.set_yticks([0,100,200,300,400])
# ax1.axvline(pd.to_datetime(df.date[151]), color = 'black', ls = '-.')
# ax1.set_ylabel('Height (arcsec)')
# ax1.tick_params('x', labelbottom=False)

# ax2= fig.add_subplot(312, sharex=ax1)
# ax2.plot(pd.to_datetime(df.date), df.height_km/1000, '.')
# # ax2.set_ylim(0,300)
# ax2.axvline(pd.to_datetime(df.date[151]), color = 'black', ls = '-.')
# ax2.set_ylabel('Height (Mm)')

# ax3 = fig.add_subplot(313, sharex=ax1)
# ax3.plot(pd.to_datetime(df.date), abs(df.velocity), '.')
# ax3.axvline(pd.to_datetime(df.date[151]), color = 'black', ls = '-.')
# ax3.set_ylabel('Velocity (km/s)')

# ax3.xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
# ax3.xaxis.set_major_locator(mdates.HourLocator(interval=2))
# # plt.gca().xaxis.set_major_locator(mdates.HourLocator(interval=2))
# plt.xlabel('Start time = 2012/03/11 19:48 UT')

# # plt.savefig('Results/height-time_plot/fig 6 Su 2015.png', dpi=200)
# plt.show()
